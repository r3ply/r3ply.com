...
export const onRequest: PagesFunction<Env> = async (context) => {
  return context.request.formData()
    .then(form_data => {
      ...
      let stmt = ps.bind(entries.email, ts, cfRay, xRealIp, cf.continent, cf.country, cf.city, cf.isEUCountry, cf.botManagement["score"], JSON.stringify(cf))
      ...
      return stmt.run()
        ...
        .catch(e => {
          ...
          let stmt = ps.bind(entries.email, ts, cfRay, e.message, e.cause.message, cf.botManagement.score, JSON.stringify(cf))
          return stmt.run()
            .then(_ => Response.redirect(url.toString(), 302))
            .catch(e => {
              console.error({ message: e.message, cause: e.cause.message })
